...
class HctsInventoryService
{
    public function getInventoryData(array $filters): array
    {
        ...
        if ($ta)
            $receiptsRaw->where('tahun_anggaran', $ta);
        if ($te)
            $receiptsRaw->where('emisi', $te);
        ...
        if ($ta)
            $submissionsRaw->where('hcts_submissions.tahun_anggaran', $ta);
        if ($te)
            $submissionsRaw->where('hcts_submissions.tahun_emisi', $te);
        ...
        $inventory = $receipts->map(function ($r) use ($submissions) {
            ...
            return (object) [
                ...
                'tahun_emisi' => $r->tahun_emisi,
                'total_received' => (int) $r->total_received,
                'total_submitted' => $totalSub,
                'stock' => (int) $r->total_received - $totalSub,
            ];
        })->values();
