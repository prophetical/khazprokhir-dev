...
<head>
    ...
    <style>
        ...
        body { font-family: 'Inter', system-ui, -apple-system, sans-serif; background-color: #f9fafb; transition: background-color 0.3s; }
        .table-tight th, .table-tight td { padding: 8px 12px; }
    </style>
</head>
<body class="p-4 md:p-10">
    <div class="print-container max-w-5xl mx-auto bg-white p-8 border border-gray-100 shadow-sm rounded-3xl min-h-screen relative overflow-hidden">
        {{-- Dekorasi premium --}}
        <div class="absolute top-0 right-0 w-48 h-48 rounded-bl-[100px] opacity-[0.08] pointer-events-none" style="background: linear-gradient(135deg, #4f46e5 0%, #06b6d4 100%);"></div>
        <div class="absolute top-0 right-0 w-24 h-24 rounded-bl-[50px] opacity-[0.12] pointer-events-none" style="background: linear-gradient(135deg, #4f46e5 0%, #06b6d4 100%);"></div>

        <!-- Action Toolbar -->
        <div class="no-print flex justify-between items-center mb-8 pb-6 border-b border-gray-100">
            <a href="{{ route('bahan-penolong.persediaan') }}" class="text-sm font-bold text-gray-400 hover:text-indigo-600 flex items-center transition-all">
            ...
            <button onclick="window.print()" class="bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-3 rounded-2xl font-black text-sm shadow-xl shadow-indigo-100 transition-all flex items-center uppercase tracking-widest">
        ...
        <header class="flex justify-between items-start mb-10">
            <div>
                ...
                <p class="text-[11px] text-gray-400 font-black uppercase tracking-[0.2em] mb-6">Khazprokhir Inventory System</p>
                ...
                <div class="flex items-center gap-3">
                    <div class="bg-gray-50 px-4 py-2 rounded-xl border border-gray-100 flex items-center">
                        ...
                        <span class="text-xs font-black text-gray-700 uppercase">Per Tanggal {{ \Carbon\Carbon::now()->locale('id')->isoFormat('D MMMM YYYY') }}</span>
                    </div>
                </div>
            </div>
            ...
            <table class="w-full text-left border-collapse table-tight">
                <thead class="bg-gray-50/50 border-b border-gray-100">
                    <tr>
                        ...
                        <th class="text-[10px] font-black text-gray-400 uppercase tracking-widest text-center">Min. Stok</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr>
                        ...
                        <td class="text-[10px] font-mono font-bold text-gray-500 uppercase">{{ $m->kode_material ?: '-' }}</td>
                        <td class="text-sm font-black text-center {{ $m->stok <= $m->min_stok ? 'text-rose-600' : 'text-indigo-600' }}">
                    ...
                    @empty
                    <tr>
                        <td colspan="7" class="text-center py-12 text-gray-400 text-xs italic">Tidak ada data material.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <footer class="mt-20 pt-10 border-t border-dashed border-gray-200 grid grid-cols-2 gap-20">
            <div class="text-center">
                ...
                <p class="text-xs font-black text-gray-900 uppercase underline decoration-gray-200 underline-offset-4">{{ auth()->user()->name }}</p>
            </div>
            <div class="text-center">
