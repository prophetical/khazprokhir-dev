    ...
    <div class="py-12" x-data="{ 
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div
                ...
                <form action="{{ route('bahan-penolong.penerimaan') }}" method="GET" class="space-y-6">
                    ...
                    <div class="flex flex-col md:flex-row gap-4 pt-2">
                        ...
                        <a href="{{ route('bahan-penolong.export', array_merge(request()->all(), ['tipe' => 'masuk'])) }}"
                        ...
                        <a href="{{ route('bahan-penolong.print', array_merge(request()->all(), ['tipe' => 'masuk', 'autoprint' => 1])) }}"
                            target="_blank"
                            class="flex-1 md:flex-none px-6 py-3 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 text-gray-700 dark:text-slate-300 text-xs font-black rounded-2xl hover:bg-gray-50 dark:hover:bg-slate-800 transition-all flex items-center justify-center gap-2 uppercase tracking-widest">
                        ...
                        @if(auth()->user()->role !== 'supervisor')
                            <button type="button" @click="showModal = true"
                                class="flex-1 md:flex-none px-8 py-3 bg-emerald-500 hover:bg-emerald-600 text-white text-xs font-black rounded-2xl shadow-lg shadow-emerald-100 dark:shadow-none transition-all flex items-center justify-center gap-2 uppercase tracking-widest">
                            ...
                            @forelse($transactions as $t)
                                <tr class="hover:bg-gray-50/50 dark:hover:bg-slate-700/30 transition-colors group">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex flex-col">
                                            <span
                                                class="font-bold text-indigo-600 dark:text-indigo-400">{{ $t->bahanPenolong->nama_bahan }}</span>
                                            <span
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <div class="inline-flex items-baseline gap-1">
                                            <span
                                                class="text-sm font-black text-emerald-600">+{{ number_format($t->jumlah) }}</span>
                                            <span
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <span
                                            class="text-xs font-bold text-gray-600 dark:text-slate-400">{{ number_format($t->stok_akhir) }}</span>
                                    </td>
                                    <td class="px-6 py-4">
                                        <span
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-2">
                                            <div
                                                class="w-6 h-6 rounded-lg bg-indigo-100 dark:bg-indigo-900/40 flex items-center justify-center text-[10px] font-black text-indigo-600 dark:text-indigo-400 uppercase">
            ...
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div x-show="showModal" x-transition:enter="transition ease-out duration-300"
                    ...
                    <form action="{{ route('bahan-penolong.transaction.store') }}" method="POST">
                        ...
                        <input type="hidden" name="kategori" value="penerimaan">
                        <div class="px-8 pt-8 pb-6">
                            <div class="flex items-center gap-4 mb-8">
                                <div
                                    class="w-12 h-12 bg-emerald-50 dark:bg-emerald-900/30 rounded-2xl flex items-center justify-center text-emerald-600 dark:text-emerald-400">
                                ...
                                <div>
                                    <h3 class="text-xl font-black text-gray-900 dark:text-white leading-none">Pencatatan
                                        Barang Datang</h3>
                                    <p class="text-sm text-gray-400 dark:text-slate-500 mt-1">Tambahkan stok baru ke
                                        inventori</p>
                                </div>
                            </div>
                            <div class="space-y-5">
                                <div class="space-y-1.5">
                                    ...
                                    <select name="bahan_penolong_id" required
                                        ...
                                        @foreach($materials as $m)
                                            <option value="{{ $m->id }}">{{ $m->nama_bahan }} ({{ $m->satuan }})</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="space-y-1.5">
                        ...
                        <div
                            ...
                            <button type="submit"
                                class="w-full sm:w-auto px-8 py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white text-xs font-black rounded-2xl shadow-lg shadow-emerald-100 dark:shadow-none transition-all">Simpan
                                Penerimaan</button>
                        </div>
                    </form>
                </div>
            ...
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div x-show="showEditModal" x-transition:enter="transition ease-out duration-300"
                    ...
                    <form :action="`/bahan-penolong/transaction/${editTransaction.id}`" method="POST">
                        @csrf @method('PUT')
                        <input type="hidden" name="kategori" :value="editTransaction.kategori">
                        <div class="px-8 pt-8 pb-6">
                            <div class="flex items-center gap-4 mb-8">
                                <div
                            ...
                            <div class="space-y-5">
                                <div class="space-y-1.5">
                                    <label
                                        class="block text-[11px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest ml-1">Jumlah
                                        Masuk (<span x-text="editTransaction.unit"></span>)</label>
