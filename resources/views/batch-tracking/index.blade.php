<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-5">
            ...
            <div x-data="{ 
                ...
                <form id="filterForm" action="{{ route('batch-tracking.index') }}" method="GET">
                    <div class="px-6 py-4 border-b border-gray-100 flex items-center gap-2">
                        ...
                        @if($search || $startDate || $endDate || $pecahanFilter)
                            <a href="{{ route('batch-tracking.index') }}"
                                class="ml-auto text-[10px] font-bold text-red-500 uppercase tracking-widest flex items-center gap-1 transition-colors hover:text-red-700">
                    ...
                    <div class="px-6 py-6 flex flex-wrap gap-6 items-end">
                        ...
                        <div class="flex-1 min-w-[220px]">
                            <label for="search"
                                ...
                                Batch / Seri</label>
                            <div class="relative">
                                <input id="search" name="search" type="text" value="{{ $search }}" placeholder="Cari..."
                    ...
                    {{-- Tombol Filter Pecahan --}}
                    <input type="hidden" name="pecahan" x-model="selectedPecahan">
                    <div class="px-6 pb-6 pt-2">
                        <div class="flex flex-wrap items-center gap-2">
                            <span class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mr-2">Filter
                                Pecahan:</span>
                            @foreach(['S', 'T', 'U', 'V', 'W', 'X', 'Y'] as $pec)
                                <button type="button" @click="setPecahan('{{ $pec }}')"
                                    ...
                                    <span
                                        class="text-[8px] font-bold leading-none mt-1 opacity-80 uppercase tracking-tighter">{{ $themeClasses[$pec]['label'] }}</span>
                                </button>
                            @endforeach
                        </div>
                    </div>
                    ...
                    @if($search || $startDate || $endDate || $pecahanFilter)
                        <div class="px-6 pb-4 flex flex-wrap gap-2">
                            @if($search)
                                <span
                                    class="inline-flex items-center gap-1 px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-indigo-50 text-indigo-700 border border-indigo-200">
                                    Keyword: <strong class="font-mono ml-1">{{ strtoupper($search) }}</strong>
                                </span>
                            @endif
                            @if($startDate)
                                <span
                                    class="inline-flex items-center gap-1 px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-amber-50 text-amber-700 border border-amber-200">
                                    Dari: <span
                                        class="ml-1">{{ \Carbon\Carbon::parse($startDate)->translatedFormat('d M Y') }}</span>
                                </span>
                            @endif
                            @if($pecahanFilter)
                                @php
                                ...
                                @endphp
                                <span
                                    class="inline-flex items-center gap-1 px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-wider {{ $bgColor }} {{ $textColor }} brightness-95">
                                    Pecahan: <strong class="ml-1">{{ $pecahanFilter }} ·
                                        {{ $themeClasses[$pecahanFilter]['label'] }}</strong>
                                </span>
                            @endif
                            @if($endDate)
                                <span
                                    class="inline-flex items-center gap-1 px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-amber-50 text-amber-700 border border-amber-200">
                                    Sampai: <span
                                        class="ml-1">{{ \Carbon\Carbon::parse($endDate)->translatedFormat('d M Y') }}</span>
                                </span>
                            @endif
                        </div>
                    @endif
            ...
            @else
                ...
                <div class="flex items-center justify-between px-1">
                    <span class="text-sm text-gray-500">Menampilkan <strong
                            class="text-gray-700">{{ $paginator->firstItem() }}–{{ $paginator->lastItem() }}</strong> dari
                        <strong class="text-gray-700">{{ $paginator->total() }}</strong> kombinasi batch/seri</span>
                    {{-- Keterangan --}}
                    <div class="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs">
                        <span class="flex items-center gap-1.5"><span
                                class="inline-block w-3.5 h-3.5 rounded bg-blue-200 border border-blue-400"></span><span
                                class="text-gray-600">Cutpack</span></span>
                        <span class="flex items-center gap-1.5"><span
                                class="inline-block w-3.5 h-3.5 rounded bg-green-200 border border-green-400"></span><span
                                class="text-gray-600">Rikyet</span></span>
                        <span class="flex items-center gap-1.5"><span
                                class="inline-block w-3.5 h-3.5 rounded bg-gray-100 border border-gray-300"></span><span
                                class="text-gray-600">Belum diinput</span></span>
                    </div>
                </div>
                ...
                @foreach($trackingData as $idx => $item)
                    ...
                    <div class="bg-white shadow-sm sm:rounded-xl overflow-visible border border-gray-100">
                        ...
                        {{-- Card Header --}}
                        <div
                            class="px-6 py-4 bg-gradient-to-r from-slate-50 to-white border-b border-gray-100 flex flex-wrap items-center gap-3">
                            <div class="flex items-center gap-3 min-w-0">
                                ...
                                <div class="flex flex-wrap items-center gap-x-3 gap-y-0.5">
                                    <div class="flex items-center gap-1.5">
                                        ...
                                        <span
                                            class="font-mono font-bold text-gray-800 text-base">{{ strtoupper($item['batch']) }}</span>
                                    </div>
                                    <span class="text-gray-300">/</span>
                                    <div class="flex items-center gap-1.5">
                                        <span class="text-xs text-gray-400 uppercase tracking-wide">Seri</span>
                                        <span
                                            class="font-mono font-bold text-indigo-700 text-base">{{ strtoupper($item['seri']) }}</span>
                                    </div>
                                </div>
                            </div>
                            <div class="flex flex-wrap items-center gap-2 ml-auto">
                                <span
                                    class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-indigo-50 text-indigo-700">{{ $item['total_packs'] }}
                                    pack</span>
                                @if($item['cutpack'] > 0)
                                    <span
                                        class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-blue-100 text-blue-700"><span
                                            class="w-1.5 h-1.5 rounded-full bg-blue-400 inline-block"></span>Cutpack:
                                        {{ $item['cutpack'] }}</span>
                                @endif
                                @if($item['rikyet'] > 0)
                                    <span
                                        class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold bg-green-100 text-green-700"><span
                                            class="w-1.5 h-1.5 rounded-full bg-green-400 inline-block"></span>Rikyet:
                                        {{ $item['rikyet'] }}</span>
                                @endif
                                <span class="text-xs text-gray-400">Bilyet: <strong
                                        class="text-gray-600">{{ number_format($item['total_jumlah'], 0, ',', '.') }}</strong></span>
                            </div>
                        </div>
                        ...
                        <div class="px-6 py-3 bg-gray-50/50 border-b border-gray-100 flex flex-wrap items-center gap-2">
                            ...
                            @foreach($item['pecahan_groups'] as $pecahan => $packMap)
                                ...
                                <div id="{{ $modalId }}"
                                    ...
                                    <div
                                        class="bg-white rounded-2xl shadow-2xl w-full max-w-4xl flex flex-col max-h-[95vh] border-t-8 {{ $themeClasses[$pecahan]['border'] }}">

                                        {{-- Modal Header --}}
                                        <div
                                            class="flex items-center justify-between px-8 py-6 border-b border-gray-100 flex-shrink-0 bg-slate-50/50 rounded-t-2xl">
                                            <div class="flex items-center gap-4">
                                                <div
                                                    class="p-3 rounded-xl {{ $themeClasses[$pecahan]['bg'] }} shadow-lg shadow-inner">
                                                ...
                                                <div>
                                                    ...
                                                    <div
                                                        ...
                                                        <span class="flex items-center gap-1.5">Batch: <span
                                                                class="text-gray-700 font-mono">{{ strtoupper($item['batch']) }}</span></span>
                                                        <span class="text-gray-300">/</span>
                                                        <span class="flex items-center gap-1.5">Seri: <span
                                                                class="text-indigo-600 font-mono">{{ strtoupper($item['seri']) }}</span></span>
                                                        <span class="text-gray-300">/</span>
                                                        <span>Pecahan: <span
                                                                class="{{ $themeClasses[$pecahan]['text'] }} {{ $themeClasses[$pecahan]['bg'] }} px-2 py-0.5 rounded text-[10px]">{{ $pecahan }}</span></span>
                                                    </div>
                                                </div>
                                            </div>
                                            <button type="button" onclick="closeModal('{{ $modalId }}')"
                                                class="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all duration-300">
                                        ...
                                        {{-- Tata Letak Modal: Sidebar Info + Grid --}}
                                        <div class="flex-1 overflow-hidden flex flex-col lg:flex-row">

                                            {{-- Sidebar: Legend & Stats --}}
                                            <div
                                                class="w-full lg:w-64 bg-gray-50/50 border-r border-gray-100 p-5 overflow-y-auto space-y-5 flex-shrink-0">

                                                {{-- Legend Group --}}
                                                <div>
                                                    <h5 class="text-[9px] font-bold text-gray-400 uppercase tracking-widest mb-3">
                                                        Keterangan Warna</h5>
                                                    <div class="space-y-2.5">
                                                        <div class="flex items-center gap-2.5">
                                                            <div
                                                                class="w-7 h-7 rounded-lg bg-blue-100 border-2 border-blue-300 shadow-sm flex-shrink-0">
                                                            </div>
                                                            <div class="flex flex-col">
                                                                <span
                                                                    class="text-[10px] font-black text-blue-800 leading-none">CUTPACK</span>
                                                                <span
                                                                    class="text-[9px] font-medium text-blue-400 mt-0.5 uppercase">Belum
                                                                    Sortir</span>
                                                            </div>
                                                        </div>
                                                        <div class="flex items-center gap-2.5">
                                                            <div
                                                                class="w-7 h-7 rounded-lg bg-blue-600 border-2 border-blue-800 shadow-md flex-shrink-0">
                                                            </div>
                                                            <div class="flex flex-col">
                                                                <span
                                                                    class="text-[10px] font-black text-blue-900 leading-none">CUTPACK</span>
                                                                <span
                                                                    class="text-[9px] font-bold text-blue-600 mt-0.5 uppercase">Sudah
                                                                    Sortir</span>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div class="flex items-center gap-2.5">
                                                            <div
                                                                class="w-7 h-7 rounded-lg bg-green-100 border-2 border-green-300 shadow-sm flex-shrink-0">
                                                            </div>
                                                            <div class="flex flex-col">
                                                                <span
                                                                    class="text-[10px] font-black text-green-800 leading-none">RIKYET</span>
                                                                <span
                                                                    class="text-[9px] font-medium text-green-400 mt-0.5 uppercase">Belum
                                                                    Sortir</span>
                                                            </div>
                                                        </div>
                                                        <div class="flex items-center gap-2.5">
                                                            <div
                                                                class="w-7 h-7 rounded-lg bg-green-600 border-2 border-green-800 shadow-md flex-shrink-0">
                                                            </div>
                                                            <div class="flex flex-col">
                                                                <span
                                                                    class="text-[10px] font-black text-green-900 leading-none">RIKYET</span>
                                                                <span
                                                                    class="text-[9px] font-bold text-green-600 mt-0.5 uppercase">Sudah
                                                                    Sortir</span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                ...
                                                <div>
                                                    ...
                                                    <div class="grid grid-cols-1 gap-1.5">
                                                        <div class="bg-white p-2.5 rounded-lg border border-gray-100 shadow-sm">
                                                            <div class="text-[8px] font-bold text-gray-400 uppercase">Input / Total
                                                            </div>
                                                            <div class="text-base font-black text-gray-900">{{ $pFilled }} <span
                                                                    class="text-[9px] text-gray-400 font-medium">/ 100</span></div>
                                                        </div>
                                                        <div class="bg-indigo-600 p-2.5 rounded-lg shadow-md">
                                                            <div class="text-[8px] font-bold text-indigo-200 uppercase">Sudah Sortir
                                                            </div>
                                                            <div class="text-base font-black text-white">{{ $pSorted }} <span
                                                                    class="text-[9px] text-indigo-300 font-medium lowercase">packs</span>
                                                            </div>
                                                        </div>
                                                        <div class="bg-white p-2.5 rounded-lg border border-gray-100 shadow-sm">
                                                            <div class="text-[8px] font-bold text-gray-400 uppercase">
                                                                SORTIR/PENERIMAAN</div>
                                                ...
                                                <div class="grid grid-cols-10 gap-1.2 w-full max-w-fit mx-auto">
                                                    @for($r = 0; $r < 10; $r++)
                                                        @for($c = 0; $c < 10; $c++)
                                                            ...
                                                            @endphp
                                                            <div
                                                                class="relative group w-8 h-8 flex items-center justify-center rounded-lg border-2 font-black text-[10px] cursor-default select-none transition-all duration-300 hover:scale-110 hover:z-50 {{ $cellClass }}">
                                                                {{ $p }}
                                                                @if($cell)
                                                                    <div
                                                                        class="pointer-events-none absolute {{ $vClass }} {{ $hClass }} z-[100] hidden group-hover:flex items-center">
                                                                        <div
                                                                            class="bg-gray-900/95 backdrop-blur-sm text-white text-[10px] rounded-xl px-3 py-2 whitespace-nowrap shadow-2xl text-center leading-tight border border-white/10">
                                                                            <div
                                                                                class="font-black border-b border-white/20 pb-1.5 mb-1.5 flex items-center justify-center gap-2">
                                                                                PACK {{ $p }}
                                                                                @if(!empty($cell['sorted']))
                                                                                    <span
                                                                                        class="px-2 py-0.5 rounded-full bg-green-500 text-[8px] text-white">TERSORTIR</span>
                                                                                @endif
                                                                            </div>
                                                                            <div
                                                                                class="font-bold {{ ($cell['supplier'] ?? '') === 'Cutpack' ? 'text-blue-300' : 'text-green-300' }} uppercase tracking-tighter">
                                                                                {{ $cell['supplier'] ?? '' }}</div>
                                        ...
                                        {{-- Footer Modal --}}
                                        <div
                                            class="px-8 py-4 border-t border-gray-100 flex justify-between items-center bg-gray-50/50 rounded-b-2xl flex-shrink-0">
                        ...
                        {{-- Detail Pack yang Dapat Diperluas --}}
                        @if($item['total_packs'] > 0)
                            <div id="{{ $cardId }}" class="hidden px-6 py-5 border-t border-gray-50">
                                <div class="mb-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">
                                    Semua pack — Batch <span class="font-mono text-gray-700">{{ strtoupper($item['batch']) }}</span>
                                    / Seri <span class="font-mono text-indigo-600">{{ strtoupper($item['seri']) }}</span>
                                </div>
                                <div class="flex flex-wrap gap-2">
                                    @foreach($item['packs'] as $pack)
                                        ...
                                        @endphp
                                        <div
                                            class="relative group cursor-default select-none w-11 h-11 flex items-center justify-center rounded-lg border-2 font-bold text-sm transition-all {{ $colorCls }}">
                                            {{ $pack->pack_number }}
                                            <div
                                                class="pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 mb-2 z-50 hidden group-hover:flex flex-col items-center">
                                                <div
                                                    class="bg-gray-900 text-white text-xs rounded-lg px-3 py-2 whitespace-nowrap shadow-xl text-center leading-relaxed">
                                ...
                                <div class="mt-3 pt-3 border-t border-gray-100 flex flex-wrap gap-4 text-xs text-gray-500">
                                    ...
                                    @if($item['cutpack'] > 0)<span class="text-blue-600">Cutpack:
                                    <strong>{{ $item['cutpack'] }}</strong></span>@endif
                                    @if($item['rikyet'] > 0)<span class="text-green-600">Rikyet:
                                    <strong>{{ $item['rikyet'] }}</strong></span>@endif
                                    <span>Total Bilyet: <strong
                                            class="text-gray-700">{{ number_format($item['total_jumlah'], 0, ',', '.') }}</strong></span>
                                </div>
                            </div>
                        @endif
                ...
                @if($paginator->hasPages())
                    <div class="flex items-center justify-between px-1">
                        <p class="text-sm text-gray-500">
                            Halaman <strong>{{ $paginator->currentPage() }}</strong> dari
                        ...
                        <div class="flex items-center gap-1">
                            ...
                            @if($paginator->onFirstPage())
                                <span
                                    class="inline-flex items-center px-3 py-1.5 text-sm text-gray-300 bg-white border border-gray-200 rounded-md cursor-not-allowed">
                                    &laquo; Sebelumnya
                                </span>
                            @else
                                <a href="{{ $paginator->previousPageUrl() }}"
                                    class="inline-flex items-center px-3 py-1.5 text-sm text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 transition-colors">
                                    &laquo; Sebelumnya
                                </a>
                            @endif
                            ...
                            @foreach($paginator->getUrlRange(max(1, $paginator->currentPage() - 2), min($paginator->lastPage(), $paginator->currentPage() + 2)) as $pageNum => $url)
                                @if($pageNum == $paginator->currentPage())
                                    <span
                                        class="inline-flex items-center justify-center w-9 h-9 text-sm font-semibold text-white bg-indigo-600 border border-indigo-600 rounded-md">
                                        {{ $pageNum }}
                                    </span>
                                @else
                                    <a href="{{ $url }}"
                                        class="inline-flex items-center justify-center w-9 h-9 text-sm text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 transition-colors">
                                        {{ $pageNum }}
                                    </a>
                                @endif
                            @endforeach
    ...
    @push('css')
        <style>
            /* Penyempurnaan Dark Mode untuk halaman batch tracking */
            ...
            /* Gradasi Kartu Header - Perbaikan untuk dark mode*/
    ...
    @push('scripts')
        <script>
            ...
            // Input pencarian huruf besar otomatis
