    ...
    <div class="py-12" x-data="dashboardData()">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
                {{-- System Status Card --}}
                <div class="lg:col-span-1 bg-white rounded-[2.5rem] shadow-xl shadow-gray-200/20 border border-gray-100 flex flex-col justify-between overflow-hidden">
                    ...
                    <div class="p-8 space-y-5">
                        ...
                        <div class="py-4 border-y border-gray-50">
                            ...
                            <div class="flex items-center gap-3">
                                <div class="w-8 h-8 bg-indigo-100 rounded-xl flex items-center justify-center text-indigo-600 font-black text-xs">
                                    {{ substr(auth()->user()->name, 0, 1) }}
                                </div>
                                <div>
                                    <p class="text-xs font-black text-gray-800">{{ auth()->user()->name }}</p>
                                    <span class="text-[9px] font-black text-indigo-500 uppercase tracking-widest">{{ auth()->user()->role }}</span>
                                </div>
                            </div>
                        </div>
                        <div x-data="{ 
                    ...
                    @foreach($modules as $mod)
                        <a href="{{ route($mod['route']) }}"
                           class="{{ $mod['bg'] }} flex flex-col items-center justify-center p-8 rounded-[2.5rem] border border-gray-100 hover:border-{{ $mod['color'] }}-400 hover:shadow-2xl hover:shadow-{{ $mod['color'] }}-100/50 hover:-translate-y-2 transition-all duration-500 group">
                            <div class="mb-5 flex items-center justify-center w-16 h-16 rounded-[1.5rem] bg-{{ $mod['color'] }}-50 text-{{ $mod['color'] }}-600 group-hover:scale-110 group-hover:rotate-6 transition-all duration-500 shadow-inner">
                                <svg class="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $mod['icon'] }}"/>
                                </svg>
                            </div>
                            <span class="text-[10px] font-black text-gray-800 uppercase tracking-[0.2em] group-hover:text-{{ $mod['color'] }}-600 transition-colors">{{ $mod['label'] }}</span>
                        </a>
                    @endforeach
                </div>
            </div>
