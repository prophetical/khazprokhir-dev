                            ...
                            <thead class="bg-gray-50">
                                ...
                                <tr>
                                    @foreach($headers as $header)
                                        <th scope="col" class="px-6 py-4 text-left text-[10px] font-bold text-gray-400 uppercase tracking-widest {{ $header === 'Aksi' ? 'text-center' : '' }}">
                                            {{ $header }}
                                        </th>
                                    @endforeach
                                </tr>
                                ...
                                @forelse($batches as $batch)
                                    <tr class="hover:bg-indigo-50/30 transition-colors">
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold border {{ $colorMap[$batch->pecahan] ?? 'bg-gray-100 text-gray-800' }}">
                                                {{ $batch->pecahan }}
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900">{{ $batch->batch }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600 italic font-medium">{{ $batch->seri }}</td>
