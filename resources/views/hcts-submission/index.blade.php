<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            @if(session('success'))
                <div class="p-4 bg-emerald-50 border-l-4 border-emerald-500 text-emerald-700 shadow-sm rounded-r-lg flex items-center gap-3" role="alert">
            ...
            @if(auth()->user()->role == 'sortir' || auth()->user()->role == 'admin')
                <div class="flex justify-end">
                    <a href="{{ route('hcts-submission.create') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white text-[9px] font-black uppercase tracking-widest rounded-xl hover:bg-emerald-700 transition-all duration-200 shadow-lg shadow-emerald-100/50">
                ...
                <div class="p-6">
                    <form action="{{ route('hcts-submission.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
                        ...
                        <div class="flex flex-col md:flex-row gap-2 h-auto md:h-[42px]">
                            <button type="submit" class="flex-1 px-4 py-2 bg-gray-900 text-white text-[10px] font-black tracking-widest rounded-xl hover:bg-gray-800 transition-all duration-200 flex items-center justify-center gap-2">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path>
                                </svg>
                                Filter
                            </button>
                            <a href="{{ route('hcts-submission.index') }}" class="px-4 py-2 bg-white border border-gray-200 text-gray-400 hover:text-rose-600 rounded-xl transition-all duration-200 flex items-center justify-center">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="mt-6 pt-6 border-t border-gray-100 flex flex-wrap gap-2 justify-end">
                        ...
                        <a href="{{ route('hcts-submission.print', request()->all()) }}" 
                           target="_blank"
                            ...
                            @forelse($submissions as $submission)
                                <tr class="hover:bg-gray-50/50 transition-colors group text-[11px]">
                                    ...
                                    <td class="px-2 py-3 text-center">
                                        ...
                                        @endphp
                                        <span class="inline-flex items-center justify-center w-7 h-7 rounded-lg {{ $currentClass }} font-black text-[10px] shadow-sm">
                                            {{ $submission->pecahan }}
                                        </span>
                                    </td>
                                    ...
                                    <td class="px-4 py-3">
                                        <div class="flex flex-wrap gap-1">
                                            @foreach($submission->batches as $batch)
                                                <span class="inline-flex items-center px-1.5 py-0.5 bg-gray-50 text-gray-600 rounded-md text-[9px] font-black border border-gray-100">
                                                    {{ $batch->batch }} 
                                                    <span class="ml-0.5 text-gray-400 font-bold">({{ number_format($batch->jumlah, 0, ',', '.') }})</span>
                                                </span>
                                            @endforeach
                                        </div>
                                    </td>
                                    <td class="px-3 py-3">
                                        <div class="flex items-center justify-center gap-1.5">
                                            <div class="w-6 h-6 rounded bg-gray-900 flex items-center justify-center text-[9px] font-black text-white">
                                                {{ strtoupper(substr($submission->user->name ?? '?', 0, 1)) }}
                                            </div>
                                            <span class="text-[9px] font-black text-gray-500 uppercase truncate max-w-[60px]">{{ $submission->user->name ?? '-' }}</span>
                                        </div>
                                    </td>
                                    <td class="px-3 py-3 text-center whitespace-nowrap">
                                        <div class="flex items-center justify-center gap-1">
                                            @if(auth()->user()->role == 'sortir' || auth()->user()->role == 'admin')
                                                <a href="{{ route('hcts-submission.edit', $submission) }}" class="p-1.5 text-indigo-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all" title="Edit">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                ...
                                                <form action="{{ route('hcts-submission.destroy', $submission) }}" method="POST" class="inline-block delete-confirm">
                                                    ...
                                                    <button type="submit" class="p-1.5 text-rose-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-all" title="Hapus">
                                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-4v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v2m3 4h.01" />
                                                        </svg>
                                                    </button>
                                                </form>
                                            @endif
                                ...
                                <tr>
                                    <td colspan="8" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center">
                                            ...
                                            <p class="text-xs font-black text-gray-400 uppercase tracking-widest italic">Belum ada data penyerahan ditemukan</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
