                        ...
                        @foreach($reportData as $row)
                            <tr x-show="search === '' || '{{ $row['pecahan'] }}'.toLowerCase().includes(search.toLowerCase())"
                                ...
                                <td class="px-4 py-4 text-center sticky left-0 bg-white group-hover:bg-indigo-50/40 z-10 border-r border-gray-100">
                                    <span class="inline-flex items-center justify-center h-7 w-7 rounded-lg shadow-sm font-black text-xs border {{ $colorMap[$row['pecahan']] ?? 'bg-gray-900 text-white' }}">
                                        {{ $row['pecahan'] }}
                                    </span>
                                </td>
                    ...
                    <tbody class="bg-white divide-y divide-gray-100">
                        @foreach(['S', 'T', 'U', 'V', 'W', 'X', 'Y'] as $pec)
                            ...
                            <tr class="hover:bg-gray-50 transition duration-150 divide-x divide-gray-100">
                                <td class="px-4 py-4 text-center sticky left-0 bg-white group-hover:bg-gray-50 z-10">
                                    <span class="inline-flex items-center justify-center h-7 w-7 rounded-lg shadow-sm font-black text-xs border {{ $colorMap[$pec] ?? 'bg-gray-900 text-white' }}">
                                        {{ $pec }}
                                    </span>
                                </td>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-100">
                        @foreach($targetAchievementData['data'] as $row)
                            <tr class="hover:bg-gray-50 transition duration-150 divide-x divide-gray-100">
                                ...
                                <td class="px-4 py-4 text-right text-xs font-bold text-gray-600 italic">{{ $row['target_bilyet'] == 0 ? '-' : number_format($row['target_bilyet'], 0, ',', '.') }}</td>
                                <td class="px-4 py-4 text-right text-xs font-bold text-indigo-700 bg-indigo-50/10">{{ $row['target_dus'] == 0 ? '-' : number_format($row['target_dus'], 0, ',', '.') }}</td>
                                <td class="px-4 py-4 text-right text-xs font-bold text-gray-600 italic">{{ $row['akumulasi_bilyet'] == 0 ? '-' : number_format($row['akumulasi_bilyet'], 0, ',', '.') }}</td>
                                <td class="px-4 py-4 text-right text-xs font-bold text-emerald-700 bg-emerald-50/10">{{ $row['akumulasi_dus'] == 0 ? '-' : number_format($row['akumulasi_dus'], 0, ',', '.') }}</td>
                                <td class="px-4 py-4 text-right text-xs font-black {{ $row['sisa_bilyet'] < 0 ? 'text-emerald-600' : ($row['sisa_bilyet'] > 0 ? 'text-rose-600' : 'text-gray-400') }}">
                                    @if($row['sisa_bilyet'] < 0) +{{ number_format(abs($row['sisa_bilyet']), 0, ',', '.') }} @elseif($row['sisa_bilyet'] > 0) {{ number_format($row['sisa_bilyet'], 0, ',', '.') }} @else - @endif
                                </td>
                                <td class="px-4 py-4 text-right text-xs font-black {{ $row['sisa_dus'] < 0 ? 'text-emerald-600' : ($row['sisa_dus'] > 0 ? 'text-rose-600' : 'text-gray-400') }}">
                                    @if($row['sisa_dus'] < 0) +{{ number_format(abs($row['sisa_dus']), 0, ',', '.') }} @elseif($row['sisa_dus'] > 0) {{ number_format($row['sisa_dus'], 0, ',', '.') }} @else - @endif
                                </td>
                                <td class="px-4 py-4 text-center">
                    ...
                    <tfoot class="bg-gray-900 text-white font-black text-xs divide-x divide-gray-700">
                        <tr class="divide-x divide-gray-700">
                            ...
                            <td class="px-4 py-6 text-right">{{ $targetAchievementData['totals']['sisa_bilyet'] == 0 ? '-' : number_format($targetAchievementData['totals']['sisa_bilyet'], 0, ',', '.') }}</td>
                            <td class="px-4 py-6 text-right">{{ $targetAchievementData['totals']['sisa_dus'] == 0 ? '-' : number_format($targetAchievementData['totals']['sisa_dus'], 0, ',', '.') }}</td>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-100">
                        @foreach($monthlyTargetAchievementData['data'] as $row)
                            <tr class="hover:bg-gray-50 transition duration-150 divide-x divide-gray-100">
                                ...
                                <td class="px-4 py-4 text-right text-xs font-black {{ $row['sisa_dus'] < 0 ? 'text-emerald-600' : ($row['sisa_dus'] > 0 ? 'text-rose-600' : 'text-gray-400') }}">
                                    @if($row['sisa_dus'] < 0) +{{ number_format(abs($row['sisa_dus']), 2, ',', '.') }} @elseif($row['sisa_dus'] > 0) {{ number_format($row['sisa_dus'], 2, ',', '.') }} @else - @endif
                                </td>
                                <td class="px-4 py-4 text-center">
                                    <div class="flex flex-col items-center gap-1">
                                        <span class="text-xs font-black {{ $row['persen'] >= 100 ? 'text-emerald-600' : ($row['persen'] >= 50 ? 'text-amber-600' : 'text-rose-600') }}">
                    ...
                    <tfoot class="bg-gray-900 text-white font-black text-xs divide-x divide-gray-700">
                        <tr class="divide-x divide-gray-700">
                            ...
                            <td class="px-4 py-6 text-right">{{ $monthlyTargetAchievementData['totals']['sisa_bilyet'] == 0 ? '-' : number_format($monthlyTargetAchievementData['totals']['sisa_bilyet'], 0, ',', '.') }}</td>
                            <td class="px-4 py-6 text-right">{{ $monthlyTargetAchievementData['totals']['sisa_dus'] == 0 ? '-' : number_format($monthlyTargetAchievementData['totals']['sisa_dus'], 0, ',', '.') }}</td>
                            <td class="px-4 py-6 text-center text-amber-400">
