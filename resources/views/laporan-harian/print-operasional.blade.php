...
<body class="p-4 md:p-10">
    <div class="print-container max-w-7xl mx-auto bg-white p-8 border border-gray-100 shadow-sm rounded-2xl min-h-screen relative overflow-hidden">
        ...
        <header class="flex justify-between items-start mb-8">
            <div>
                <h1 class="text-2xl font-extrabold text-gray-900 mb-1 uppercase tracking-tight text-indigo-900">LAPORAN HARIAN OPERASIONAL - {{ \Carbon\Carbon::parse($tanggal_laporan)->locale('id')->isoFormat('D MMMM YYYY') }}</h1>
            ...
            <div class="text-right">
                ...
                <p class="text-xs font-bold text-indigo-600">{{ \Carbon\Carbon::now()->locale('id')->isoFormat('D MMMM YYYY, HH:mm') }}</p>
            </div>
        </header>
            ...
            <table class="w-full text-left border-collapse table-tight">
                <thead>
                    ...
                    <tr class="text-[8px] lowercase text-gray-400 bg-gray-50/50">
                        ...
                        <th class="text-center italic">k=i/j*100%</th>
        ...
        @if(request('showMonitoring', 'true') === 'true')
        <div class="mb-10">
            ...
            <table class="w-full text-left border-collapse table-tight">
                <thead>
                    <tr class="bg-gray-100 text-[10px] font-black uppercase text-gray-600">
                        ...
                        <th rowspan="2" class="text-center bg-amber-50 text-amber-700">Target Harian<br><span class="text-[9px] font-bold text-amber-500 italic">{{ $sisaHariKerja }} Hari Kerja</span></th>
                        <th colspan="4" class="text-center bg-emerald-50 text-emerald-700">Produksi Pengemasan (Bilyet)<br><span class="text-[9px] font-bold text-emerald-500 italic">{{ \Carbon\Carbon::parse($tanggal_laporan)->subDay()->locale('id')->isoFormat('dddd, D MMMM Y') }}</span></th>
                        <th colspan="3" class="text-center bg-purple-50 text-purple-700">Penerimaan HCS (Bilyet)<br><span class="text-[9px] font-bold text-purple-500 italic">{{ \Carbon\Carbon::parse($tanggal_laporan)->subDay()->locale('id')->isoFormat('dddd, D MMMM Y') }}</span></th>
                    </tr>
                    <tr class="bg-gray-50 text-[9px] font-bold uppercase text-gray-500">
                ...
                <tbody class="text-xs font-medium text-gray-700 divide-y divide-gray-200">
                    @foreach($secondaryData as $row)
                        <tr>
                            ...
                            <td class="text-right">{{ $row['target_penyerahan_bulan'] == 0 ? '-' : number_format($row['target_penyerahan_bulan'], 0, ',', '.') }}</td>
                            <td class="text-right">{{ $row['penyerahan_bulan'] == 0 ? '-' : number_format($row['penyerahan_bulan'], 0, ',', '.') }}</td>
                            <td class="text-right font-black {{ $row['sisa_target_bilyet'] < 0 ? 'text-green-600' : 'text-blue-700' }}">{{ $row['sisa_target_bilyet'] == 0 ? '-' : ($row['sisa_target_bilyet'] < 0 ? '+' : '') . number_format(abs($row['sisa_target_bilyet']), 0, ',', '.') }}</td>
                            <td class="text-right font-bold text-indigo-600 bg-indigo-50/20 italic">{{ $row['sisa_target_doos'] == 0 ? '-' : ($row['sisa_target_doos'] < 0 ? '+' : '') . number_format(abs($row['sisa_target_doos']), 0, ',', '.') }}</td>
                            <td class="text-right font-bold text-amber-600 bg-amber-50/20">{{ $row['target_produksi_harian'] == 0 ? '-' : number_format($row['target_produksi_harian'], 0, ',', '.') }}</td>
                            <td class="text-right">{{ $row['kemas_g1'] == 0 ? '-' : number_format($row['kemas_g1'], 0, ',', '.') }}</td>
                            <td class="text-right">{{ $row['kemas_g2'] == 0 ? '-' : number_format($row['kemas_g2'], 0, ',', '.') }}</td>
                            <td class="text-right">{{ $row['kemas_g3'] == 0 ? '-' : number_format($row['kemas_g3'], 0, ',', '.') }}</td>
                            <td class="text-right font-black text-emerald-700 bg-emerald-50/20">{{ $row['total_kemas'] == 0 ? '-' : number_format($row['total_kemas'], 0, ',', '.') }}</td>
                            <td class="text-right">{{ $row['hcs_rikyet'] == 0 ? '-' : number_format($row['hcs_rikyet'], 0, ',', '.') }}</td>
                            <td class="text-right">{{ $row['hcs_cutpack'] == 0 ? '-' : number_format($row['hcs_cutpack'], 0, ',', '.') }}</td>
                            <td class="text-right font-black text-purple-700 bg-purple-50/20">{{ $row['total_hcs'] == 0 ? '-' : number_format($row['total_hcs'], 0, ',', '.') }}</td>
                        </tr>
                    @endforeach
                </tbody>
                <tfoot class="bg-gray-900 text-white text-[10px] font-black uppercase">
                    <tr>
                        <td class="text-center py-3">TOTAL</td>
                        <td class="text-right">{{ $secondaryTotals['target_penyerahan_bulan'] == 0 ? '-' : number_format($secondaryTotals['target_penyerahan_bulan'], 0, ',', '.') }}</td>
                        <td class="text-right">{{ $secondaryTotals['penyerahan_bulan'] == 0 ? '-' : number_format($secondaryTotals['penyerahan_bulan'], 0, ',', '.') }}</td>
                        <td class="text-right {{ $secondaryTotals['sisa_target_bilyet'] < 0 ? 'text-green-400' : 'text-blue-300' }}">{{ $secondaryTotals['sisa_target_bilyet'] == 0 ? '-' : ($secondaryTotals['sisa_target_bilyet'] < 0 ? '+' : '') . number_format(abs($secondaryTotals['sisa_target_bilyet']), 0, ',', '.') }}</td>
                        <td class="text-right text-indigo-300">{{ $secondaryTotals['sisa_target_doos'] == 0 ? '-' : ($secondaryTotals['sisa_target_doos'] < 0 ? '+' : '') . number_format(abs($secondaryTotals['sisa_target_doos']), 0, ',', '.') }}</td>
                        <td class="text-right text-amber-300">{{ $secondaryTotals['target_produksi_harian'] == 0 ? '-' : number_format($secondaryTotals['target_produksi_harian'], 0, ',', '.') }}</td>
                        <td class="text-right">{{ $secondaryTotals['kemas_g1'] == 0 ? '-' : number_format($secondaryTotals['kemas_g1'], 0, ',', '.') }}</td>
                        <td class="text-right">{{ $secondaryTotals['kemas_g2'] == 0 ? '-' : number_format($secondaryTotals['kemas_g2'], 0, ',', '.') }}</td>
                        <td class="text-right">{{ $secondaryTotals['kemas_g3'] == 0 ? '-' : number_format($secondaryTotals['kemas_g3'], 0, ',', '.') }}</td>
                        <td class="text-right text-emerald-300">{{ $secondaryTotals['total_kemas'] == 0 ? '-' : number_format($secondaryTotals['total_kemas'], 0, ',', '.') }}</td>
                        <td class="text-right">{{ $secondaryTotals['hcs_rikyet'] == 0 ? '-' : number_format($secondaryTotals['hcs_rikyet'], 0, ',', '.') }}</td>
                        <td class="text-right">{{ $secondaryTotals['hcs_cutpack'] == 0 ? '-' : number_format($secondaryTotals['hcs_cutpack'], 0, ',', '.') }}</td>
                        <td class="text-right text-purple-300">{{ $secondaryTotals['total_hcs'] == 0 ? '-' : number_format($secondaryTotals['total_hcs'], 0, ',', '.') }}</td>
                    </tr>
                </tfoot>
            </table>
        </div>
            ...
            <table class="w-full text-left border-collapse table-tight">
                <thead>
                    <tr class="bg-gray-100 text-[10px] font-black uppercase text-gray-600">
                        ...
                        <th class="text-center">Penerimaan<br><span class="text-[8px] italic font-normal text-gray-400">{{ \Carbon\Carbon::parse($tanggal_laporan)->subDay()->locale('id')->isoFormat('dddd, D MMMM Y') }}</span></th>
                        <th class="text-center bg-emerald-50 text-emerald-700">Penyerahan<br><span class="text-[8px] italic font-normal text-emerald-400">{{ \Carbon\Carbon::parse($tanggal_laporan)->locale('id')->isoFormat('dddd, D MMMM Y') }}</span></th>
                        <th class="text-center">Akumulasi Penerimaan</th>
                        <th class="text-center">Akumulasi Penyerahan</th>
                        <th class="text-center bg-indigo-50 text-indigo-700">Persediaan HCTS</th>
                    ...
                    <tr class="text-[8px] lowercase text-gray-400 bg-gray-50/50">
                        ...
                        <th class="text-center italic">g=f/3.000.000</th>
                    </tr>
                </thead>
                <tbody class="text-xs font-medium text-gray-700 divide-y divide-gray-200">
                    @foreach(['S', 'T', 'U', 'V', 'W', 'X', 'Y'] as $pec)
                        ...
                        <tr>
                            <td class="text-center bg-gray-50 p-1">
                                <span class="inline-flex items-center justify-center w-6 h-6 rounded shadow-sm text-[10px] font-black border {{ $colorMap[$pec] ?? 'bg-gray-700 text-white' }}">
                                    {{ $pec }}
                                </span>
                            </td>
                            <td class="text-right">{{ $data['penerimaan_h1'] == 0 ? '-' : number_format($data['penerimaan_h1'], 0, ',', '.') }}</td>
                            <td class="text-right font-black text-emerald-700 bg-emerald-50/10">{{ $data['penyerahan_hari_ini'] == 0 ? '-' : number_format($data['penyerahan_hari_ini'], 0, ',', '.') }}</td>
                            <td class="text-right">{{ $data['akumulasi_penerimaan'] == 0 ? '-' : number_format($data['akumulasi_penerimaan'], 0, ',', '.') }}</td>
                            <td class="text-right">{{ $data['akumulasi_penyerahan'] == 0 ? '-' : number_format($data['akumulasi_penyerahan'], 0, ',', '.') }}</td>
                            <td class="text-right font-black text-indigo-700 bg-indigo-50/20">{{ $data['persediaan'] == 0 ? '-' : number_format($data['persediaan'], 0, ',', '.') }}</td>
                            <td class="text-center font-black text-amber-700">{{ $data['ct_siap_hitung'] == 0 ? '-' : $data['ct_siap_hitung'] . ' CT' }}</td>
                        </tr>
                    @endforeach
                </tbody>
                <tfoot class="bg-gray-900 text-white text-[10px] font-black uppercase">
                    ...
                    <tr>
                        <td class="text-center py-3">TOTAL</td>
                        <td class="text-right">{{ $totalPenerimaanH1 == 0 ? '-' : number_format($totalPenerimaanH1, 0, ',', '.') }}</td>
                        <td class="text-right text-emerald-400">{{ $totalPenyerahanHariIni == 0 ? '-' : number_format($totalPenyerahanHariIni, 0, ',', '.') }}</td>
                        <td class="text-right">{{ $totalAkumulasiTerima == 0 ? '-' : number_format($totalAkumulasiTerima, 0, ',', '.') }}</td>
                        <td class="text-right">{{ $totalAkumulasiSerah == 0 ? '-' : number_format($totalAkumulasiSerah, 0, ',', '.') }}</td>
                        <td class="text-right text-indigo-300">{{ $totalPersediaan == 0 ? '-' : number_format($totalPersediaan, 0, ',', '.') }}</td>
        ...
        @if(request('showAnnual', 'true') === 'true')
        <div class="mb-10">
            ...
            <table class="w-full text-left border-collapse table-tight">
                <thead>
                    <tr class="bg-gray-100 text-[10px] font-black uppercase text-gray-600">
                        ...
                        <th colspan="2" class="text-center bg-indigo-50 text-indigo-700">Target TA {{ $tahun_anggaran }}</th>
                ...
                <tbody class="text-xs font-medium text-gray-700 divide-y divide-gray-200">
                    @foreach($targetAchievementData['data'] as $row)
                        <tr>
                            ...
                            <td class="text-right">{{ $row['target_bilyet'] == 0 ? '-' : number_format($row['target_bilyet'], 0, ',', '.') }}</td>
                            <td class="text-right font-bold text-indigo-700 bg-indigo-50/10">{{ $row['target_dus'] == 0 ? '-' : number_format($row['target_dus'], 0, ',', '.') }}</td>
                            <td class="text-right">{{ $row['akumulasi_bilyet'] == 0 ? '-' : number_format($row['akumulasi_bilyet'], 0, ',', '.') }}</td>
                            <td class="text-right font-bold text-emerald-700 bg-emerald-50/10">{{ $row['akumulasi_dus'] == 0 ? '-' : number_format($row['akumulasi_dus'], 0, ',', '.') }}</td>
                            <td class="text-right font-black {{ $row['sisa_bilyet'] < 0 ? 'text-emerald-600' : ($row['sisa_bilyet'] > 0 ? 'text-rose-600' : 'text-gray-400') }}">
                                {{ $row['sisa_bilyet'] == 0 ? '-' : ($row['sisa_bilyet'] < 0 ? '+' : '') . number_format(abs($row['sisa_bilyet']), 0, ',', '.') }}
                            </td>
                            <td class="text-right font-black {{ $row['sisa_dus'] < 0 ? 'text-emerald-600' : ($row['sisa_dus'] > 0 ? 'text-rose-600' : 'text-gray-400') }}">
                                {{ $row['sisa_dus'] == 0 ? '-' : ($row['sisa_dus'] < 0 ? '+' : '') . number_format(abs($row['sisa_dus']), 0, ',', '.') }}
                            </td>
                            <td class="text-center font-black {{ $row['persen'] >= 100 ? 'text-green-600' : ($row['persen'] >= 50 ? 'text-amber-600' : 'text-rose-600') }}">
                ...
                <tfoot class="bg-gray-900 text-white text-[10px] font-black uppercase">
                    <tr>
                        <td class="text-center py-3">TOTAL</td>
                        <td class="text-right">{{ number_format($targetAchievementData['totals']['target_bilyet'], 0, ',', '.') }}</td>
                        <td class="text-right text-indigo-300">{{ number_format($targetAchievementData['totals']['target_dus'], 0, ',', '.') }}</td>
                        <td class="text-right">{{ number_format($targetAchievementData['totals']['akumulasi_bilyet'], 0, ',', '.') }}</td>
                        <td class="text-right text-emerald-300">{{ number_format($targetAchievementData['totals']['akumulasi_dus'], 0, ',', '.') }}</td>
                        <td class="text-right {{ $targetAchievementData['totals']['sisa_bilyet'] < 0 ? 'text-emerald-400' : 'text-rose-400' }}">
                            {{ ($targetAchievementData['totals']['sisa_bilyet'] < 0 ? '+' : '') . number_format(abs($targetAchievementData['totals']['sisa_bilyet']), 0, ',', '.') }}
                        </td>
                        <td class="text-right {{ $targetAchievementData['totals']['sisa_dus'] < 0 ? 'text-emerald-400' : 'text-rose-400' }}">
        ...
        @if(request('showMonthly', 'true') === 'true')
        <div class="mb-10">
            <h3 class="text-lg font-black text-rose-700 mb-4 flex items-center">
                <span class="w-1.5 h-5 bg-rose-600 rounded-full mr-2"></span>
                Laporan Pencapaian Target Pengemasan Bulanan ({{ \Carbon\Carbon::parse($tanggal_laporan)->locale('id')->isoFormat('MMMM Y') }})
            </h3>
            <table class="w-full text-left border-collapse table-tight">
                <thead>
                    <tr class="bg-gray-100 text-[10px] font-black uppercase text-gray-600">
                        <th rowspan="2" class="text-center bg-gray-200">PEC</th>
                        <th colspan="2" class="text-center bg-indigo-50 text-indigo-700">Target Bulan {{ \Carbon\Carbon::parse($tanggal_laporan)->locale('id')->isoFormat('MMMM') }}</th>
                        <th colspan="2" class="text-center bg-emerald-50 text-emerald-700">Akumulasi Bulan {{ \Carbon\Carbon::parse($tanggal_laporan)->locale('id')->isoFormat('MMMM') }}</th>
                        <th colspan="2" class="text-center bg-rose-50 text-rose-700">Sisa / Over</th>
                        <th rowspan="2" class="text-center bg-amber-50 text-amber-700">% Pencapaian</th>
                    </tr>
                    <tr class="bg-gray-50 text-[9px] font-bold uppercase text-gray-500">
                ...
                <tfoot class="bg-gray-900 text-white text-[10px] font-black uppercase">
                    <tr>
                        <td class="text-center py-3">TOTAL</td>
                        <td class="text-right">{{ number_format($monthlyTargetAchievementData['totals']['target_bilyet'], 0, ',', '.') }}</td>
                        <td class="text-right text-indigo-300">{{ number_format($monthlyTargetAchievementData['totals']['target_dus'], 0, ',', '.') }}</td>
                        <td class="text-right">{{ number_format($monthlyTargetAchievementData['totals']['akumulasi_bilyet'], 0, ',', '.') }}</td>
                        <td class="text-right text-emerald-300">{{ number_format($monthlyTargetAchievementData['totals']['akumulasi_dus'], 0, ',', '.') }}</td>
                        <td class="text-right {{ $monthlyTargetAchievementData['totals']['sisa_bilyet'] < 0 ? 'text-emerald-400' : 'text-rose-400' }}">
                            {{ ($monthlyTargetAchievementData['totals']['sisa_bilyet'] < 0 ? '+' : '') . number_format(abs($monthlyTargetAchievementData['totals']['sisa_bilyet']), 0, ',', '.') }}
                        </td>
                        <td class="text-right {{ $monthlyTargetAchievementData['totals']['sisa_dus'] < 0 ? 'text-emerald-400' : 'text-rose-400' }}">
