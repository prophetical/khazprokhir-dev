...
<head>
    ...
    <title>Laporan Rekonsiliasi Data HCS -
        {{ \Carbon\Carbon::parse($start_date)->locale('id')->isoFormat('D MMMM YYYY') }} s/d
        {{ \Carbon\Carbon::parse($end_date)->locale('id')->isoFormat('D MMMM YYYY') }}
    </title>
...
<body class="p-4 md:p-10">
    <div
        ...
        <div class="no-print flex justify-between items-center mb-8 pb-6 border-b border-gray-100">
            <a href="{{ route('laporan-harian.rekonsiliasi', request()->all()) }}"
                ...
                Kembali ke Halaman Rekonsiliasi
            </a>
        ...
        <header class="flex justify-between items-start mb-8">
            <div>
                <h1 class="text-2xl font-extrabold text-indigo-900 mb-1 uppercase tracking-tight">LAPORAN REKONSILIASI
                    DATA HCS</h1>
                ...
                <div class="flex flex-wrap items-center gap-3 text-xs mb-4">
                    <div class="bg-indigo-50 px-3 py-1.5 rounded-lg border border-indigo-100 flex items-center">
                        <span class="text-indigo-400 mr-2 uppercase font-bold text-[10px]">Periode:</span>
                        <span class="font-bold text-indigo-700 underline decoration-indigo-200 underline-offset-4">
                            {{ \Carbon\Carbon::parse($start_date)->locale('id')->isoFormat('D MMMM YYYY') }} —
                            {{ \Carbon\Carbon::parse($end_date)->locale('id')->isoFormat('D MMMM YYYY') }}
                        </span>
                    </div>
                    @if($tahun_anggaran)
        ...
        <div class="mb-10">
            <h3 class="text-lg font-black text-indigo-900 mb-4 flex items-center">
                ...
                Data Rekonsiliasi HCS per Pecahan
            </h3>
            <table class="w-full text-left border-collapse table-tight">
                <thead>
                    <tr class="bg-gray-100 text-[10px] font-black uppercase text-gray-600">
                        <th rowspan="2" class="text-center bg-gray-200 w-16 align-middle">Pecahan</th>
                        <th rowspan="2" class="text-center bg-amber-50/30 text-amber-900 align-middle">Penerimaan HCS
                            (Bilyet)</th>
                        <th rowspan="2" class="text-center bg-emerald-50/30 text-emerald-900 align-middle">Pengemasan
                            HCS (Bilyet)</th>
                        <th colspan="2" class="text-center bg-emerald-50/20 text-emerald-900 align-middle">Range No. Dus
                            Pengemasan</th>
                        <th rowspan="2" class="text-center bg-rose-50/30 text-rose-900 align-middle">Penyerahan HCS
                            (Bilyet)</th>
                        <th colspan="2" class="text-center bg-rose-50/20 text-rose-900 align-middle">Range No. Dus
                            Penyerahan</th>
                        <th rowspan="2" class="text-center bg-blue-50/30 text-blue-900 align-middle">Akumulasi Target
                            Pengemasan</th>
                        <th rowspan="2" class="text-center bg-indigo-50/30 text-indigo-900 align-middle">Akumulasi
                            Target Penyerahan</th>
                    </tr>
                    <tr class="bg-gray-50 text-[10px] font-black uppercase text-gray-600">
                        <th class="text-center bg-emerald-50/10 text-emerald-900">No Awal</th>
                        <th class="text-center bg-emerald-50/10 text-emerald-900">No Akhir</th>
                        <th class="text-center bg-rose-50/10 text-rose-900">No Awal</th>
                        <th class="text-center bg-rose-50/10 text-rose-900">No Akhir</th>
                    </tr>
                </thead>
                <tbody class="text-xs font-medium text-gray-700 divide-y divide-gray-200">
                    @foreach($rekonsiliasiData as $row)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="text-right font-semibold text-gray-600">{{ $row['min_dus_kemas'] }}</td>
                            <td class="text-right font-semibold text-gray-600">{{ $row['max_dus_kemas'] }}</td>
                            <td class="text-right font-semibold">
                                {{ $row['penyerahan_hcs'] > 0 ? number_format($row['penyerahan_hcs'], 0, ',', '.') : '-' }}
                            </td>
                            <td class="text-right font-semibold text-gray-600">{{ $row['min_dus_serah'] }}</td>
                            <td class="text-right font-semibold text-gray-600">{{ $row['max_dus_serah'] }}</td>
                ...
                <tfoot class="bg-gray-900 text-white text-xs font-black uppercase">
                    <tr>
                        ...
                        <td class="text-right text-gray-400 font-bold">-</td>
                        <td class="text-right text-gray-400 font-bold">-</td>
