        ...
        @if(auth()->user()->role)
            ...
            <div class="space-y-1 mt-2">
                <a href="{{ route('hcs-khazai-registration.index') }}"
                    class="flex items-center py-2.5 px-3 rounded-lg transition-all duration-200 {{ request()->routeIs('hcs-khazai-registration.*') ? 'bg-white/20 text-white font-semibold shadow-sm' : 'text-white/70 hover:bg-white/10 hover:text-white' }}"
                    title="Registrasi HCS (Khazai)">
                    <div class="shrink-0 w-8 flex justify-center">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <span x-show="!sidebarCollapsed" x-transition
                        class="ml-3 text-sm font-medium whitespace-nowrap overflow-hidden">Registrasi HCS</span>
                </a>
            </div>
        @endif
        ...
        @if(auth()->user()->role)
            ...
            <div class="space-y-1">
                ...
                <div x-show="hcsOpen && !sidebarCollapsed" x-transition:enter="transition ease-out duration-200"
                    x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0"
                    class="pl-11 space-y-1">
                    @if(in_array(auth()->user()->role, ['admin', 'sortir', 'kemas']))
                        <a href="{{ route('hcs-receiving.scan') }}"
                            class="block py-2 text-[13px] transition-all duration-200 {{ request()->routeIs('hcs-receiving.scan') ? 'text-white font-bold' : 'text-white/60 hover:text-white' }}">
                            Scan Barcode HCS
                        </a>
                    @endif
