                    ...
                    <div class="p-6">
                        ...
                        <form method="GET" action="{{ route('pengemasan.data') }}">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
                                ...
                                <div>
                                    <label
                                        class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2 px-1">Tanggal
                                        Awal</label>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end mt-4">
                                ...
                                <div>
                                    ...
                                    <div class="relative">
                                        <span class="absolute inset-y-0 left-0 flex items-center pl-3">
                                            <svg class="w-4 h-4 transition-colors duration-500"
                                                :class="currentTheme ? currentTheme.text.replace('text-white', 'text-indigo-400') : 'text-indigo-400'"
                                ...
                                <div class="flex flex-col gap-2">
                                    <div class="flex gap-2 h-full">
                                        <button type="submit"
                                            class="flex-1 inline-flex justify-center items-center px-4 py-3 border border-transparent rounded-lg font-bold text-xs text-white uppercase tracking-widest shadow-md active:scale-95 transition-all duration-500"
                            ...
                            <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-800">
                                <thead class="bg-gray-100 dark:bg-slate-800">
                                    <tr>
                                        ...
                                        <th
                                            class="px-2 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider bg-gray-50 border-b border-gray-200">
                                            <a href="{{ route('pengemasan.data', array_merge(request()->query(), ['sort' => 'gilir', 'direction' => $direction])) }}"
                                                class="hover:text-indigo-600 flex items-center">
                                                Gilir {{ sortIcon('gilir') }}
                                            </a>
                                        </th>
                                        <th
                                            class="px-4 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider bg-gray-50 border-b border-gray-200">
                                            <a href="{{ route('pengemasan.data', array_merge(request()->query(), ['sort' => 'pecahan', 'direction' => $direction])) }}"
                                                class="hover:text-indigo-600 flex items-center">
                                                Identitas {{ sortIcon('pecahan') }}
                                            </a>
                                        </th>
                                        <th
                                            class="px-6 py-4 text-center text-xs font-bold text-gray-700 uppercase tracking-wider bg-gray-50 border-b border-gray-200">
                                            <a href="{{ route('pengemasan.data', array_merge(request()->query(), ['sort' => 'pack_awal', 'direction' => $direction])) }}"
                                                class="hover:text-indigo-600 flex items-center justify-center">
                                                Pack {{ sortIcon('pack_awal') }}
                                            </a>
                                        </th>
                                        <th
                                            class="px-2 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider bg-gray-50 border-b border-gray-200">
                                            <a href="{{ route('pengemasan.data', array_merge(request()->query(), ['sort' => 'jumlah_dus', 'direction' => $direction])) }}"
                                                class="hover:text-indigo-600 flex items-center">
                                    ...
                                    @forelse($pengemasans as $p)
                                        <tr class="hover:bg-indigo-50 dark:hover:bg-slate-800/50 transition-colors duration-200 group">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-center text-sm text-gray-600 dark:text-gray-300">
                                                ...
                                                <div class="text-xs text-gray-500 font-bold">({{ $p->jumlah_pack }} pack)
                                                </div>
                                            </td>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                <div class="flex items-center gap-2">
                                                    <a href="{{ route('pengemasan.show', $p) }}"
                                                        class="inline-flex items-center text-indigo-600 hover:text-indigo-900 bg-indigo-50 hover:bg-indigo-100 px-3 py-1.5 rounded-sm transition-colors duration-200 shadow-sm"
                                                        title="Lihat Detail">
                                                    ...
                                                    @if(in_array(auth()->user()->role, ['sortir', 'admin']))
                                                        <form id="delete-form-{{ $p->id }}"
                                                            action="{{ route('pengemasan.destroy', $p->id) }}" method="POST"
                                                            class="inline">
                                                            @csrf
                                                            @method('DELETE')
                                                            <button type="button" onclick="deletePengemasan({{ $p->id }})"
                                                                class="inline-flex items-center text-red-600 hover:text-red-900 bg-red-50 hover:bg-red-100 px-3 py-1.5 rounded-sm transition-colors duration-200 shadow-sm"
                                                                title="Hapus Data">
                                                                <svg class="ml-1.5 w-4 h-4" fill="none" viewBox="0 0 24 24"
                                                                    stroke="currentColor">
                                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                                        stroke-width="2"
