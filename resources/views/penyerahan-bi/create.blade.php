<x-app-layout>
    ...
    <div class="py-6">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
                    ...
                    <div class="divide-y divide-red-100">
                        @foreach($missingWarnings as $warn)
                            ...
                            <div class="px-5 py-3 flex flex-wrap items-start gap-3">
                                <span class="{{ $bgColor }} text-white text-[9px] font-black px-2 py-0.5 rounded shrink-0 mt-0.5">{{ $warn['pecahan'] }}</span>
                                <div class="flex flex-wrap gap-2 items-center flex-1">
                                    ...
                                    <span class="text-gray-300">·</span>
                                    <span class="text-[10px] font-bold text-indigo-600 font-mono">Penyerahan: {{ $warn['nomor_range'] }}</span>
                                    <span class="text-gray-300">·</span>
                                    <span class="text-[10px] font-bold text-red-500 font-mono">BA: {{ $warn['nomor_ba'] }}</span>
                                </div>
                                <div class="w-full pl-7 mt-1">
                                    <span class="text-[10px] text-red-500 font-bold">Nomor Belum Dikemas ({{ $warn['missing_count'] }} dus):</span>
                                    <span class="text-[10px] font-mono font-bold text-red-800 ml-1 bg-red-100 px-2 py-0.5 rounded">{{ $warn['missing_ranges'] }}</span>
                                </div>
                            </div>
                        @endforeach
                    </div>
            ...
            {{-- Warning dari session setelah redirect --}}
            @if(session('warning_penyerahan'))
                ...
                <div class="mb-6 bg-amber-50 border-l-4 border-amber-400 p-5 rounded-r-lg shadow-sm">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            <h3 class="font-bold text-amber-800 text-sm">Peringatan: Sebagian Nomor Dus Belum Ada di Sistem</h3>
                            <p class="text-amber-700 text-xs mt-1">Data penyerahan berhasil disimpan, namun terdapat nomor dus yang belum memiliki data pengemasan.</p>
                        ...
                        <div class="mb-5 bg-red-50 border-l-4 border-red-500 p-4 rounded-r-lg shadow-sm">
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-red-700 text-xs mt-1">Range nomor dus ini sebelumnya sudah diinput dengan identitas yang sama. Periksa kembali data Anda sebelum menyimpan.</p>
                    ...
                    <form method="POST" action="{{ route('penyerahan-bi.store') }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-5 mb-5">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <input type="text" name="nomor_ba" value="{{ old('nomor_ba') }}" placeholder="Contoh: BA-123/BI/2025"
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-5 mb-5">
                            ...
                            <div>
                                ...
                                <input type="text" inputmode="numeric"
                                    ...
                                    :class="currentTheme ? (currentTheme.focus + ' ' + currentTheme.ring) : 'focus:border-indigo-500 focus:ring-indigo-500'" required>
                                <div class="text-[10px] text-gray-400 mt-1 text-right font-mono" x-show="bilyetRaw > 0" x-text="bilyetRaw.toLocaleString('id-ID') + ' lembar'"></div>
                            </div>
                        </div>
                        ...
                        <div class="bg-indigo-50/70 border border-indigo-100 rounded-xl p-4 mb-5">
                            ...
                            <div class="grid grid-cols-3 gap-4 items-center">
                                <div>
                                    ...
                                    <input type="number" name="nomor_dus_awal" x-model.number="noAwal" value="{{ old('nomor_dus_awal') }}" min="1" placeholder="1"
                                ...
                                <div>
                                    ...
                                    <input type="number" name="nomor_dus_akhir" x-model.number="noAkhir" value="{{ old('nomor_dus_akhir') }}" min="1" placeholder="100"
                                        class="block w-full border-indigo-200 bg-white rounded-lg shadow-sm text-sm py-2.5 px-3 font-bold transition-all focus:ring-opacity-50"
                                        :class="currentTheme ? (currentTheme.focus + ' ' + currentTheme.ring) : 'focus:border-indigo-500 focus:ring-indigo-500'" required>
                                </div>
                        ...
                        <div class="flex gap-3">
                            <button type="submit"
                                ...
                                <span x-text="dupWarning ? 'Range Duplikat — Periksa Kembali' : 'Simpan Penyerahan'"></span>
                            </button>
                            <a href="{{ route('penyerahan-bi.index') }}" class="px-6 py-3 bg-gray-100 border border-gray-200 rounded-lg font-bold text-sm text-gray-600 uppercase tracking-widest shadow-sm hover:bg-gray-200 active:scale-95 transition-all">
                                Batal
                            </a>
                        </div>
                    </form>
