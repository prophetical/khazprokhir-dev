    ...
    <div class="py-8">
        <div class="max-w-full mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            @if(session('warning_penyerahan'))
                ...
                <div class="bg-amber-50 border-l-4 border-amber-400 p-5 rounded-r-lg shadow-sm">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            <h3 class="font-bold text-amber-800 text-sm">Data disimpan — Sebagian Nomor Dus Belum Ada di
                                Sistem</h3>
                ...
                <div class="bg-red-50 border border-red-200 rounded-xl shadow-sm overflow-hidden">
                    <div class="flex items-center justify-between px-5 py-3 bg-red-100 border-b border-red-200">
                        <div class="flex items-center gap-3">
                            ...
                            <h3 class="font-bold !text-black text-sm">{{ $missingWarnings->count() }} Penyerahan Belum
                                Lengkap — Nomor Dus Belum Dikemas</h3>
                        </div>
                        ...
                        @foreach($missingWarnings as $warn)
                            ...
                            <div class="px-5 py-3 grid grid-cols-1 md:grid-cols-2 gap-2">
                                <div class="flex flex-wrap items-center gap-2">
                                    <span
                                        class="{{ $bgColor }} text-white text-[9px] font-black px-2 py-0.5 rounded">{{ $warn['pecahan'] }}</span>
                                    <span class="text-[10px] font-bold text-indigo-600 font-mono">TA
                                        {{ $warn['tahun_anggaran'] }}</span>
                                    <span class="text-gray-300">·</span>
                                    <span class="text-[10px] font-bold text-indigo-600 font-mono">TE
                                        {{ $warn['tahun_emisi'] }}</span>
                                    <span class="text-gray-300">·</span>
                                    <span class="text-[10px] font-bold text-indigo-600 font-mono">Dus
                                        {{ $warn['nomor_range'] }}</span>
                                    <span class="text-gray-300">·</span>
                                    <span class="text-[10px] font-mono text-red-500 font-bold">NO BA Penyerahan:
                                        {{ $warn['nomor_ba'] }}</span>
                                </div>
                                <div class="flex items-center gap-2">
                                    <span class="text-[10px] text-red-500 font-bold shrink-0">Belum Dikemas
                                        ({{ $warn['missing_count'] }} dus):</span>
                                    <span
                                        class="text-[10px] font-mono font-bold text-red-800 bg-red-100 px-2 py-0.5 rounded break-all">{{ $warn['missing_ranges'] }}</span>
                                </div>
                            </div>
                        @endforeach
                    </div>
            ...
            <div x-data="{
                ...
                <div class="p-6">
                    <h3 class="text-lg font-bold text-gray-900 border-l-4 pl-4 mb-6 transition-colors duration-500"
                    ...
                    <form method="GET" action="{{ route('penyerahan-bi.index') }}">
                            ...
                            <div>
                                ...
                                <select name="pecahan" x-model="selectedPecahan"
                                    ...
                                    @foreach(['S', 'T', 'U', 'V', 'W', 'X', 'Y'] as $p)
                                        <option value="{{ $p }}" {{ request('pecahan') == $p ? 'selected' : '' }}>{{ $p }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end mt-4">
                            ...
                            <div class="lg:col-span-2">
                                <label
                                    class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Cari
                                    (Nomor BA, Pecahan, dll)</label>
                            ...
                            <div>
                                <label
                                    ...
                                    BA Spesifik</label>
                                <input type="text" name="nomor_ba" value="{{ request('nomor_ba') }}"
                                    placeholder="BA-xxx/BI/xxxx"
                        ...
                        {{-- Tombol export & aksi --}}
                        <div
                            class="mt-6 pt-4 border-t border-gray-100 flex flex-wrap gap-2 justify-between items-center">
                            ...
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    @foreach($cols as $col => $label)
                                        <th
                                            class="px-2 py-3 sm:px-4 text-left text-[9px] sm:text-xs font-bold text-gray-600 uppercase tracking-wider bg-gray-50 border-b border-gray-200">
                                    ...
                                    <th
                                        ...
                                        Petugas</th>
                                    <th
                                        class="px-2 py-3 sm:px-4 text-center text-[9px] sm:text-xs font-bold text-gray-600 uppercase tracking-wider bg-gray-50 border-b border-gray-200">
                                        Aksi</th>
                                </tr>
                            </thead>
                            <tbody class="bg-white divide-y divide-gray-100">
                                @forelse($penyerahans as $row)
                                    ...
                                    <tr class="hover:bg-indigo-50/30 transition-colors duration-150">
                                        <td
                                            class="px-2 py-3 sm:px-4 whitespace-nowrap text-[10px] sm:text-sm text-gray-600 font-medium">
                                            {{ \Carbon\Carbon::parse($row->tanggal_penyerahan)->locale('id')->isoFormat('D MMM YY') }}
                                        </td>
                                        <td
                                            class="px-2 py-3 sm:px-4 text-[9px] sm:text-xs font-mono font-bold text-gray-800 break-all max-w-[80px] sm:max-w-none">
                                            {{ $row->nomor_ba }}
                                        </td>
                                        <td class="px-2 py-3 sm:px-4 whitespace-nowrap">
                                            <div class="flex flex-col gap-0.5">
                                                <div class="flex items-center gap-1 sm:gap-1.5">
                                                    <span
                                                        class="{{ $c['bg'] }} text-white text-[8px] sm:text-[9px] font-bold px-1 sm:px-1.5 py-0.5 rounded">{{ $row->pecahan }}</span>
                                                    <span
                                                        class="text-[9px] sm:text-[10px] font-bold text-gray-700">{{ $row->tahun_anggaran }}</span>
                                                    <span class="text-gray-300 text-[9px] sm:text-[10px]">/</span>
                                                    <span
                                                        class="text-[9px] sm:text-[10px] font-bold text-gray-500">{{ $row->tahun_emisi }}</span>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-2 py-3 sm:px-4 whitespace-nowrap text-center">
                                            <span
                                                class="inline-flex items-center px-1.5 py-0.5 sm:px-2 rounded text-[9px] sm:text-xs font-medium bg-indigo-50 text-indigo-700 border border-indigo-100">
                                        ...
                                        <td
                                            class="px-2 py-3 sm:px-4 whitespace-nowrap text-[10px] sm:text-xs font-black text-emerald-600 text-right">
                                            {{ number_format($row->jumlah_bilyet, 0, ',', '.') }}
                                        </td>
                                        <td class="px-2 py-3 sm:px-4 whitespace-nowrap">
                                            @if($row->status_data === 'Lengkap')
                                                <span
                                                    class="inline-flex items-center px-1.5 py-0.5 sm:px-2 rounded-full text-[8px] sm:text-[10px] font-bold bg-green-100 text-green-700 border border-green-200">
                                                    ✓ Lengkap
                                                </span>
                                            @else
                                                <span
                                                    class="inline-flex items-center px-1.5 py-0.5 sm:px-2 rounded-full text-[8px] sm:text-[10px] font-bold bg-amber-100 text-amber-700 border border-amber-200">
                                                    ⚠ Belum
                                                </span>
                                            @endif
                                        </td>
                                        <td
                                            class="px-2 py-3 sm:px-4 whitespace-nowrap text-[8px] sm:text-[10px] text-gray-500">
                                            <div class="flex items-center gap-1">
                                                <div
                                                    class="h-4 w-4 rounded-full bg-gray-100 flex items-center justify-center text-[7px] sm:text-[8px] font-bold text-gray-500 border border-gray-200">
                                                    {{ substr($row->user->name ?? '?', 0, 1) }}</div>
                                                <span
                                                    class="truncate max-w-[40px] sm:max-w-[70px]">{{ $row->user->name ?? '-' }}</span>
                                            </div>
                                        </td>
                                        <td class="px-2 py-3 sm:px-4 whitespace-nowrap text-center">
                                            <div class="flex items-center justify-center gap-1 sm:gap-2">
                                                <a href="{{ route('penyerahan-bi.edit', $row->id) }}"
                                                    class="p-1 sm:p-1.5 text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors"
                                                    title="Edit Data">
                                                ...
                                                <form id="delete-form-{{ $row->id }}"
                                                    action="{{ route('penyerahan-bi.destroy', $row->id) }}" method="POST"
                                                    class="inline-block">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="button" onclick="confirmDelete({{ $row->id }})"
                                                        class="p-1 sm:p-1.5 text-rose-600 hover:bg-rose-50 rounded-lg transition-colors"
                                                        title="Hapus Data">
                                    ...
                                    <tr>
                                        <td colspan="10" class="px-4 py-8 text-center text-gray-400">
                                            <div class="flex flex-col items-center justify-center">
                                                ...
                                                <p class="text-sm font-medium">Belum ada data penyerahan untuk filter ini.
                                                </p>
                                            </div>
                                        </td>
                                    </tr>
