...
<body class="p-4 md:p-10">
    <div
        class="max-w-6xl mx-auto bg-white p-8 border border-gray-100 shadow-sm rounded-2xl min-h-screen relative overflow-hidden">
        ...
        <div class="no-print flex justify-between items-center mb-8 pb-6 border-b border-gray-100">
            <a href="{{ route('penyerahan-bi.index', request()->all()) }}"
            ...
            <button onclick="window.print()"
                class="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2.5 rounded-xl font-bold text-sm shadow-lg transition-all flex items-center">
                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <header class="flex justify-between items-start mb-8">
            <div>
                <h1 class="text-2xl font-extrabold text-gray-900 mb-1 tracking-tight">LAPORAN PENYERAHAN KE BANK
                    INDONESIA</h1>
            ...
            <div class="border border-gray-200 rounded-xl overflow-hidden shadow-sm">
                <table class="w-full text-left border-collapse table-tight">
                        ...
                        @forelse($penyerahans as $row)
                            <tr>
                                ...
                                <td class="text-center">
                                    <span
                                        class="{{ $colorMap[$row->pecahan] ?? 'bg-gray-500' }} text-white text-[9px] font-bold px-1.5 py-0.5 rounded">{{ $row->pecahan }}</span>
                                </td>
                                ...
                                <td class="text-[10px] font-medium text-gray-600 text-center bg-indigo-50/50 rounded px-2">
                                    {{ $row->nomor_dus_awal }} – {{ $row->nomor_dus_akhir }}</td>
                                ...
                                <td class="text-center">
                                    @if($row->status_data === 'Lengkap')
                                        <span class="text-[9px] font-bold text-green-700 bg-green-100 px-1.5 py-0.5 rounded">✓
                                            Lengkap</span>
                                    @else
                                        <span class="text-[9px] font-bold text-amber-700 bg-amber-100 px-1.5 py-0.5 rounded">⚠
                                            Belum</span>
                                    @endif
                                </td>
                    ...
                    <tfoot class="bg-gray-50 border-t border-gray-200">
                        <tr>
                            <td colspan="6" class="text-[10px] font-bold text-gray-700 uppercase p-3 text-right">Total
                            </td>
                            <td class="text-[10px] font-extrabold text-purple-700 text-right p-3">
                                {{ number_format($penyerahans->sum('jumlah_dus'), 0, ',', '.') }}</td>
                            <td class="text-[10px] font-extrabold text-emerald-600 text-right p-3">
                                {{ number_format($penyerahans->sum('jumlah_bilyet'), 0, ',', '.') }}</td>
                            <td colspan="2"></td>
                        </tr>
                    </tfoot>
                </table>
