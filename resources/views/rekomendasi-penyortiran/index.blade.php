            ...
            :class="currentTheme ? currentTheme.border : 'border-transparent'">
                <div class="p-6">
                    <div class="flex items-center mb-4">
                        <div class="w-8 h-8 rounded-lg flex items-center justify-center mr-3 shadow-sm transition-all duration-500"
                            ...
                            <svg class="w-5 h-5 transition-colors duration-500" 
                                 :class="currentTheme ? currentTheme.text : 'text-indigo-600'"
                        ...
                        <h3 class="text-sm font-bold text-gray-700 uppercase tracking-widest">
                            ...
                            <template x-if="selectedPecahan">
                                <span>- <span class="text-gray-900">Pecahan <span x-text="selectedPecahan"></span></span></span>
                            </template>
                        </h3>
                    </div>
                    ...
                    <form method="GET" action="{{ route('rekomendasi-penyortiran.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                        ...
                        <div class="relative">
                            <label for="batch" class="block text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1 ml-1">Batch</label>
                            ...
                            @forelse ($paginator as $item)
                                ...
                                @if($isFirstOfGroup)
                                    ...
                                    <tr class="bg-indigo-50/30 border-t-2 border-indigo-100">
                                        <td colspan="7" class="px-6 py-2 text-xs text-indigo-800 font-semibold text-right">
                                            {{ $item['pecahan'] }} - {{ $item['batch'] }} - {{ $item['seri'] }} - <span class="uppercase tracking-wider font-bold">{{ $item['supplier'] }}</span>
                                        </td>
                                ...
                                <tr class="{{ $rowBgClass }} hover:bg-gray-100 transition-colors">
                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-900 font-mono tracking-wide leading-relaxed">
                                        @foreach($item['ranges_str'] as $rangeStr)
                                            <span class="inline-block bg-gray-200 rounded px-2 py-0.5 mb-1 mr-1 border border-gray-300 font-medium whitespace-nowrap">{{ $rangeStr }}</span>
                                            @if(!$loop->last)
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                        <a href="{{ route('hcs-sorting.create', ['pecahan' => $item['pecahan'], 'batch' => $item['batch'], 'seri' => $item['seri']]) }}" 
                                           class="inline-flex items-center px-3 py-1.5 border border-gray-300 shadow-sm text-xs font-medium rounded text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors">
                                            Sortir
                                            <svg class="ml-1.5 w-3 h-3 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3"></path></svg>
                                        </a>
                                    </td>
                                </tr>
                            @empty
