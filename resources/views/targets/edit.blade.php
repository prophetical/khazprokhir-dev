<x-app-layout>
    ...
    <div class="py-12" x-data="{
        selectedPecahan: '{{ old('pecahan', $targetTahunan->pecahan) }}',
        themes: {{ json_encode($themeClasses) }},
        annualTarget: {{ (float) old('target_tahunan', $targetTahunan->target) }},
        monthlyTargets: {
            @foreach($months as $num => $name)
                {{ $num }}: {{ (float) old('bulan_' . $num, $targetBulanan->{"bulan_$num"}) }},
            @endforeach
        },
        pengemasanMonthlyTargets: {
            @foreach($months as $num => $name)
                {{ $num }}: {{ (float) old('pengemasan_bulan_' . $num, $targetPengemasan->{"bulan_$num"}) }},
            @endforeach
        },
        annualFormatted: '',
        monthlyFormatted: {},
                ...
                <div class="p-8 text-gray-900">
                    <div class="flex justify-between items-center mb-8">
                        <div class="flex flex-col">
                            <h3 class="text-lg font-bold text-indigo-800">Edit Data Target</h3>
                            <p class="text-xs text-gray-500 mt-1">Sesuaikan target tahunan dan rincian target per bulan.
                            </p>
                        </div>
                    ...
                    <form method="POST" action="{{ route('targets.update', $targetTahunan->id) }}">
                        @csrf
                        @method('PUT')
                        <input type="hidden" name="target_tahunan" :value="annualTarget">
                        @foreach($months as $num => $name)
                            <input type="hidden" name="bulan_{{ $num }}" :value="monthlyTargets[{{ $num }}]">
                            <input type="hidden" name="pengemasan_bulan_{{ $num }}"
                                :value="pengemasanMonthlyTargets[{{ $num }}]">
                        @endforeach
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                            ...
                            <div>
                                ...
                                <input type="number" name="tahun_anggaran"
                                    value="{{ old('tahun_anggaran', $targetTahunan->tahun_anggaran) }}"
                                    class="block w-full border-gray-200 rounded-lg shadow-sm text-sm py-2.5 px-3 transition-all font-bold focus:ring-opacity-50"
                                    :class="currentTheme ? (currentTheme.focus + ' ' + currentTheme.ring) : 'focus:border-indigo-500 focus:ring-indigo-500'"
                                    required>
                            </div>
                            <div>
                                <label
                                    class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">Tahun
                                    Emisi</label>
                                <input type="number" name="tahun_emisi"
                                    value="{{ old('tahun_emisi', $targetTahunan->tahun_emisi) }}"
                                    class="block w-full border-gray-200 rounded-lg shadow-sm text-sm py-2.5 px-3 transition-all font-bold focus:ring-opacity-50"
                                    :class="currentTheme ? (currentTheme.focus + ' ' + currentTheme.ring) : 'focus:border-indigo-500 focus:ring-indigo-500'"
                                    required>
                            </div>
                            <div>
                                <label
                                    class="block text-[10px] font-bold text-indigo-400 uppercase tracking-widest mb-2">Target
                                    Tahunan (Penyerahan)</label>
                                <input type="text" x-model="annualFormatted" @input="updateAnnual($event.target.value)"
                                    class="block w-full border-indigo-100 bg-indigo-50/30 rounded-lg shadow-sm text-sm py-2.5 px-3 transition-all font-black text-indigo-700 text-right focus:ring-indigo-500 focus:border-indigo-500"
                        ...
                        <div class="relative mb-8">
                            ...
                            <div class="relative flex justify-start">
                                <span
                                    class="pr-3 bg-white text-[10px] font-bold text-emerald-600 uppercase tracking-[0.2em]">Rincian
                                    Target Penyerahan Bulanan</span>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-x-6 gap-y-5 mb-10">
                            @foreach($months as $num => $name)
                                <div>
                                    <label
                                        class="block text-[9px] font-bold text-gray-400 uppercase tracking-wider mb-1.5">{{ $name }}</label>
                                    <input type="text" x-model="monthlyFormatted[{{ $num }}]"
                                        @input="updateMonthly({{ $num }}, $event.target.value)"
                                        class="block w-full border-gray-100 bg-gray-50/50 rounded-lg shadow-sm text-xs py-2 px-3 transition-all font-bold text-right focus:ring-emerald-300 focus:border-emerald-300"
                                        placeholder="0">
                                </div>
                            @endforeach
                        </div>
                        ...
                        <div class="relative mb-8">
                            ...
                            <div class="relative flex justify-start">
                                <span
                                    class="pr-3 bg-white text-[10px] font-bold text-indigo-600 uppercase tracking-[0.2em]">Rincian
                                    Target Pengemasan Bulanan</span>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-x-6 gap-y-5 mb-10">
                            @foreach($months as $num => $name)
                                <div>
                                    <label
                                        class="block text-[9px] font-bold text-gray-400 uppercase tracking-wider mb-1.5">{{ $name }}</label>
                                    <input type="text" x-model="pengemasanMonthlyFormatted[{{ $num }}]"
                                        @input="updatePengemasanMonthly({{ $num }}, $event.target.value)"
                                        class="block w-full border-gray-100 bg-gray-50/50 rounded-lg shadow-sm text-xs py-2 px-3 transition-all font-bold text-right focus:ring-indigo-300 focus:border-indigo-300"
                                        placeholder="0">
                                </div>
                            @endforeach
                        </div>
                        ...
                        {{-- Footer Info & Submit --}}
                        <div
                            class="bg-gray-50 rounded-xl p-5 border border-gray-100 mb-8 flex flex-col md:flex-row justify-between items-center gap-4">
                            <div class="flex items-center gap-4">
                                <div class="flex flex-col">
                                    <span class="text-[9px] font-bold text-gray-400 uppercase tracking-widest">Akumulasi
                                        Bulanan</span>
                                    <span class="text-xl font-black text-gray-700"
                                        x-text="formatRibuan(totalMonthly) || 0">0</span>
                                </div>
                                <div class="h-8 w-px bg-gray-200 hidden md:block"></div>
                                <div class="flex flex-col">
                                    <span class="text-[9px] font-bold text-gray-400 uppercase tracking-widest">Target
                                        Tahunan Ter-set</span>
                                    <span class="text-sm font-bold text-gray-500" x-text="annualFormatted || 0">0</span>
                                </div>
                            </div>
                            ...
                            <div class="flex gap-3 w-full md:w-auto">
                                ...
                                <a href="{{ route('targets.index') }}"
                                    class="flex-1 md:flex-none text-center px-8 py-3 bg-white border border-gray-200 rounded-lg font-bold text-sm text-gray-500 uppercase tracking-widest shadow-sm hover:bg-gray-50 active:scale-95 transition-all">
                                    Batal
                                </a>
                            </div>
                        </div>
