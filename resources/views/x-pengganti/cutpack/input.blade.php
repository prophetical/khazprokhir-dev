<x-app-layout :full-screen="true">
            ...
            <div class="flex items-center gap-3 min-w-0">
                ...
                <div class="min-w-0">
                    <div class="flex items-center gap-2 flex-wrap">
                        ...
                        <span class="px-2 py-0.5 bg-white/10 rounded-lg text-white/70 text-[10px] font-bold uppercase tracking-tight">Batch {{ $seri->batch }}</span>
                        <span class="px-2 py-0.5 bg-white/10 rounded-lg text-white/70 text-[10px] font-bold uppercase tracking-tight">TA {{ $seri->tahun_anggaran }}</span>
                        <span class="px-2 py-0.5 bg-white/10 rounded-lg text-white/70 text-[10px] font-bold uppercase tracking-tight">TE {{ $seri->tahun_emisi }}</span>
                    </div>
                </div>
            </div>
    ...
    <div class="h-full flex flex-col bg-slate-50 dark:bg-slate-900 overflow-hidden">
        <div class="flex-1 overflow-y-auto px-2 sm:px-3 py-3 scroll-container">
            
            {{-- Errors Flash --}}
            @if($errors->any())
                <div class="mb-3 p-3 bg-rose-50 dark:bg-rose-900/20 border-l-4 border-rose-500 text-rose-700 dark:text-rose-400 rounded-lg">
            ...
            <form id="cutpack-form" action="{{ route('x-pengganti.cutpack.store') }}" method="POST">
                ...
                <div id="page-1" class="grid grid-cols-2 gap-3">
                    {{-- ── KIRI: Pack 1–25 ── --}}
                    <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm relative overflow-hidden">
                        <div class="sticky top-0 z-30 px-3 py-1.5 bg-slate-800 text-white text-[9px] font-black uppercase tracking-[0.2em] text-center border-b border-slate-700">
                            PACK 1 — 25
                        </div>
                        <div class="overflow-x-auto">
                            <table class="w-full border-separate border-spacing-0">
                                @include('x-pengganti.cutpack._table-head')
                                <tbody>
                ...
                {{-- Float Totals & Nav footer --}}
                <div class="mt-4 flex flex-wrap items-center justify-between gap-4 p-4 bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-2xl shadow-xl shadow-slate-200/50 dark:shadow-none sticky bottom-0 z-40">
                    <div class="flex items-center gap-6 overflow-x-auto pb-1">
                        <div class="flex flex-col min-w-max">
                            <span class="text-[8px] font-black text-amber-500 uppercase tracking-widest leading-none mb-1">Total Seri 1</span>
                            <span id="grand-total-seri1" class="text-xl font-black text-slate-800 dark:text-white">0</span>
                        </div>
                        <div class="flex flex-col min-w-max">
                            <span class="text-[8px] font-black text-amber-500 uppercase tracking-widest leading-none mb-1">Total Seri 2</span>
                            <span id="grand-total-seri2" class="text-xl font-black text-slate-800 dark:text-white">0</span>
                        </div>
                        <div class="flex flex-col min-w-max">
                            <span class="text-[8px] font-black text-indigo-500 uppercase tracking-widest leading-none mb-1">Total Campuran</span>
                            <span id="grand-total-campuran" class="text-xl font-black text-slate-800 dark:text-white">0</span>
                        </div>
                    </div>
    ...
    <style>
        .page-nav-btn { background: rgba(255, 255, 255, 0.05); color: rgba(255, 255, 255, 0.4); border: 1.5px solid rgba(255, 255, 255, 0.1); }
        .page-nav-btn:hover { background: rgba(255, 255, 255, 0.1); color: white; border-color: rgba(255,255,255,0.2); }
        .page-nav-btn.active { background: #ffffff; color: #4338ca; font-weight: 900; box-shadow: 0 0 20px rgba(255,255,255,0.15); border-color: white; }

        .footer-page-btn.active { background: #4338ca; color: white !important; border-color: #4338ca !important; box-shadow: 0 4px 15px rgba(67, 56, 202, 0.3); }

        .pack-row-first td { border-top: 2px solid #6366f1 !important; }
        .dark .pack-row-first td { border-top: 2px solid #4f46e5 !important; }

        .khazai-input { transition: all 0.2s; }
        .khazai-input:focus { border-color: #6366f1 !important; background-color: rgba(99, 102, 241, 0.05) !important; box-shadow: 0 0 0 2px rgba(99, 102, 241, 0.2) !important; outline: none !important; }
        ...
        .dark .scroll-container::-webkit-scrollbar-thumb { background: #475569; }
    </style>
    ...
    <script>
        ...
        document.getElementById('cutpack-form')?.addEventListener('submit', function (e) {
            ...
            if (hasAnyData) document.getElementById('packs-json-input').value = JSON.stringify(packsData);

            // Disable original inputs to avoid max_input_vars issues
            ...
            setTimeout(() => {
                ...
                if (btn) { btn.style.pointerEvents = 'auto'; btn.style.opacity = '1'; btn.innerHTML = 'Simpan Data'; }
            }, 3000);
        });
