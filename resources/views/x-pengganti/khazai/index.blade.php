<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            <h2 class="font-black text-xl text-gray-800 dark:text-white leading-tight tracking-tight">Form Input Khazai</h2>
        </div>
    </x-slot>
        ...
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 shadow-sm overflow-hidden">
                ...
                <div class="p-5 border-b border-gray-100 dark:border-slate-700 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-[10px] text-gray-400 dark:text-slate-500 font-bold">Klik <span class="text-violet-500 font-black">Buka &amp; Input</span> — tabel pengisian akan terbuka di tab baru</p>
                        </div>
                    </div>
                    <a href="{{ route('x-pengganti.seri.create') }}"
                        class="inline-flex items-center gap-2 px-4 py-2 bg-violet-600 text-white text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-violet-700 transition-all whitespace-nowrap shrink-0">
                ...
                <div class="px-5 py-4 bg-gray-50/50 dark:bg-slate-700/20 border-b border-gray-100 dark:border-slate-700">
                    <form method="GET" action="{{ route('x-pengganti.khazai.index') }}" class="flex flex-wrap gap-3 items-end">
                        {{-- Search --}}
                        <div class="flex-1 min-w-[180px]">
                            <label class="block text-[9px] font-black tracking-widest text-violet-600 mb-1 uppercase">Cari Seri / Batch</label>
                            <div class="relative">
                                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                    <svg class="h-4 w-4 text-violet-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
                                </div>
                                <input name="search" type="text" value="{{ $search }}"
                                    placeholder="Ketik seri atau batch..."
                                    class="block w-full pl-10 pr-4 py-2 bg-white dark:bg-slate-700 border border-gray-200 dark:border-slate-600 focus:border-violet-500 rounded-xl text-xs font-bold text-gray-700 dark:text-gray-200 dark:placeholder:text-slate-500">
                            </div>
                        </div>

                        {{-- Pecahan --}}
                        <div class="min-w-[90px]">
                            <label class="block text-[9px] font-black tracking-widest text-violet-600 mb-1 uppercase">Pecahan</label>
                            <select name="pecahan" class="block w-full py-2 px-3 bg-white dark:bg-slate-700 border border-gray-200 dark:border-slate-600 focus:border-violet-500 rounded-xl text-xs font-black text-gray-700 dark:text-gray-200 text-center-last">
                        ...
                        <div class="min-w-[110px]">
                            <label class="block text-[9px] font-black tracking-widest text-violet-600 mb-1 uppercase">Tahun Anggaran</label>
                            <select name="tahun_anggaran" class="block w-full py-2 px-3 bg-white dark:bg-slate-700 border border-gray-200 dark:border-slate-600 focus:border-violet-500 rounded-xl text-xs font-black text-gray-700 dark:text-gray-200 text-center-last">
                                <option value="">Semua</option>
                        ...
                        <div class="min-w-[100px]">
                            <label class="block text-[9px] font-black tracking-widest text-violet-600 mb-1 uppercase">Tahun Emisi</label>
                            <select name="tahun_emisi" class="block w-full py-2 px-3 bg-white dark:bg-slate-700 border border-gray-200 dark:border-slate-600 focus:border-violet-500 rounded-xl text-xs font-black text-gray-700 dark:text-gray-200 text-center-last">
                                <option value="">Semua</option>
                        ...
                        <a href="{{ route('x-pengganti.khazai.index') }}" class="h-[38px] px-3 bg-white dark:bg-slate-700 border border-gray-200 dark:border-slate-600 text-gray-400 hover:text-rose-600 rounded-xl transition-all flex items-center" title="Reset">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-slate-700">
                            @forelse($masters as $i => $master)
                                <tr class="hover:bg-violet-50/40 dark:hover:bg-violet-900/10 transition-colors">
                                    ...
                                    <td class="px-4 py-3 text-center">
                                        <a href="{{ route('x-pengganti.khazai.input', ['seri_id' => $master->id]) }}"
                                            target="_blank"
                                            class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-gradient-to-r from-violet-600 to-purple-600 text-white text-[10px] font-black uppercase tracking-widest rounded-lg hover:from-violet-700 hover:to-purple-700 transition-all shadow-sm shadow-violet-200 dark:shadow-violet-900/30 whitespace-nowrap">
                                    ...
                                    <td colspan="7" class="px-6 py-14 text-center">
                                        <div class="flex flex-col items-center">
                                            <div class="w-14 h-14 rounded-2xl bg-violet-50 dark:bg-violet-900/20 flex items-center justify-center mb-3">
                                                <svg class="w-7 h-7 text-violet-200 dark:text-violet-700" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                                            </div>
                                            <p class="text-xs font-black text-gray-400 dark:text-slate-500 uppercase tracking-widest">Tidak ada data seri ditemukan</p>
                                            <a href="{{ route('x-pengganti.seri.create') }}" class="mt-4 inline-flex items-center gap-2 px-4 py-2 bg-violet-600 text-white text-[9px] font-black uppercase tracking-widest rounded-xl hover:bg-violet-700 transition-all">
